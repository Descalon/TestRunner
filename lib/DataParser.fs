...
module DataParser =
    ...
    let private padLeft width (line:string) = 
        line.PadLeft(width)
    ...
    let toSingleLine (line:RecordLine) = 
        let file = (line.Code |> string).PadLeft(2,'0')
        let map = [
        ...
        let folder (v:string, width:int) (state:string)=
            let p = (width - state.Length) + v.Length
            state + (v.PadLeft(p))

        file
        |> List.foldBack folder map
        |> padLeft 203
